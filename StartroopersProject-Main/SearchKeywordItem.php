...
$sql = "SELECT * FROM productmain2 WHERE p_name LIKE '%" . $keyword . "%' AND p_status = 'active';";
...
$sql = "SELECT * FROM productmain2 WHERE p_name LIKE '%" . $keyword . "%' AND p_status = 'active' LIMIT 12 OFFSET $recordrange";
$result = mysqli_query($conn, $sql);
...
if (isset($_POST['ASC'])) {
  ...
  header("Location:SearchKeywordItem.php?searchkeyword=$keyword&PageNum=$page&sort=$sort"); 
...
if(isset($_GET['sort']) && isset($_GET['PageNum']))
  {
    $sorting =$_GET['sort'];
    $sql = "SELECT * FROM productmain2 WHERE p_name LIKE '%" . $keyword . "%' AND p_status = 'active' ORDER BY p_price $sorting LIMIT 12 OFFSET $recordrange;";
    $result = mysqli_query($conn, $sql);
    $products = mysqli_fetch_all($result, MYSQLI_ASSOC);
  }
?>
...
if (isset($_GET['sort'])) {
  $sort = $_GET['sort'];
  $sql = "SELECT * FROM productmain2 WHERE p_name LIKE '%" . $keyword . "%' ORDER BY p_price $sort  LIMIT 12 OFFSET $recordrange ;";
  //$result = executeQuery($desc_query);
  $result = mysqli_query($conn, $sql);
  $products = mysqli_fetch_all($result, MYSQLI_ASSOC);
}
...
if (isset($_POST['search'])) {
  ...
  header("Location:SearchKeywordItem.php?searchkeyword=$keyword");
}
?>
<!DOCTYPE html>
<html lang="en">
        ...
        ?>
          ...
          <form method="">
            ...
            <!-- isset page number is equal to 0, do not display the left arrow -->
              <div style="display: none;">
                <a href="?searchkeyword=<?php echo $keyword ?>&PageNum=<?php echo $_REQUEST['PageNum'] - 1 ?>&sort=<?php echo $sort ?>"> <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-caret-left" viewBox="0 0 16 16">
            ...
            <?php } else { ?>
              ...
              <a href="?searchkeyword=<?php echo $keyword ?>&PageNum=<?php if (!isset($_GET['PageNum'])) {
      $page = 0;
    } else {
      $page = $_GET['PageNum'];
            ...
            ?>
              ...
              <div class="pagenumborder">
                <div class="pagenum<?php echo $classname ?>">
                  <a href="?searchkeyword=<?php echo $keyword ?>&PageNum=<?php echo $i ?>&sort=<?php echo $sort ?>"><?php echo $i + 1 ?></a>
                </div>
              </div>
...
<?php } else { ?>
  ...
  <form method="">
    ...
    ?>
       <!-- isset page number is equal to 0, do not display the left arrow -->
      <div style="display: none;">
        <a href="?searchkeyword=<?php echo $keyword ?>&PageNum=<?php echo $_REQUEST['PageNum'] - 1 ?>"> <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-caret-left" viewBox="0 0 16 16">
            <path d="M10 12.796V3.204L4.519 8 10 12.796zm-.659.753-5.48-4.796a1 1 0 0 1 0-1.506l5.48-4.796A1 1 0 0 1 11 3.204v9.592a1 1 0 0 1-1.659.753z" />
          </svg></a>
...
<!-- (left arrow)if page number did not set, set $page as 1, if it is set , get the page number and plus one-->
    ...
    ?>
      ...
      <div class="pagenumborder">
        <div class="pagenum<?php echo $classname ?>">
          <a href="?searchkeyword=<?php echo $keyword ?>&PageNum=<?php echo $i ?>"><?php echo $i + 1 ?></a>
        </div>
      </div>
