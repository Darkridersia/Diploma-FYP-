...
// if user didnt login, bring them to login page
  ...
  $sqltest = "SELECT * FROM productmain2 WHERE p_status = 'active';";
    //row count 
  if ($resulttest = mysqli_query($conn, $sqltest)) {
    $rowcounttest = mysqli_num_rows($resulttest);
  }
  ...
  $sql = "SELECT * FROM productmain2 WHERE p_status = 'active' LIMIT 12 OFFSET $recordrange";
  ...
  if (isset($_POST['ASC'])) {
    ...
    header("Location:index.php?PageNum=$page&sort=$sort");
 ...
 //Get the sort type and get the page number
  ...
  {
    $sorting =$_GET['sort'];
    $sql = "SELECT * FROM productmain2 WHERE p_status = 'active' ORDER BY p_price $sorting  LIMIT 12 OFFSET $recordrange";
    ...
    <div class="row">
      ...
      <?php foreach ($products as $product) : ?>
        ...
        <div class="col-12 col-lg-4">
          <!-- if $product['p_status'] is delete, dont display, if $product['p_status'] is active, display as block -->
          <div class="<?php echo $product['p_status'] ?>">
            <div class="case">
              ...
              <?php if ($qty > 0) { ?>
              <!-- if item quantity is more than 0, display this -->
              ...
              <?php } else {   ?>
               <!-- if item quantity is equal to 0. display this -->
    ...
    //  user click add to cart button
        if (isset($_REQUEST['add' . $productID])) {
          ...
          if (mysqli_num_rows($resultrow) == 1 && $stock <= $max) {
            ...
            $update_stmt = $db->prepare('UPDATE addtocart SET total_price =:c_total,quantity =:c_quantity'); 		$update_stmt->bindParam(':c_total', $totalprice);		
            ...
            try {
              ...
              if (!isset($errorMsg)) {
                $insert_stmt = $db->prepare('INSERT INTO addtocart(product_name,price,quantity,shipping,images,total_price,seller_email,seller_contactNo,product_id,cart_user_email,status) VALUES(:aname,:aprice,:aquantity,:ashipping,:aimage,:atotalprice,:aseller_email,:aseller_contactNo,:aproduct_id,:acart_user_email,:astatus)'); 
                // bind paramter				
...
// get the sort type
        ...
        ?>
          ...
          <form method="">
            ...
            <!-- isset page number is equal to 0, do not display the left arrow -->
              <div style="display: none;">
                <a href="?PageNum=<?php echo $_REQUEST['PageNum'] - 1 ?>&sort=<?php echo $sort ?>"> <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-caret-left" viewBox="0 0 16 16">
            ...
            <?php } else { ?>
             ...
             <!-- (left arrow)if page number did not set, set $page as 1, if it is set , get the page number and plus one-->
              <a href="?PageNum=<?php if (!isset($_GET['PageNum'])) {
      $page = 0;
    } else {
      $page = $_GET['PageNum'];
    } echo $page - 1 ?>&sort=<?php echo $sort ?>"> <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-caret-left" viewBox="0 0 16 16">
                  <path d="M10 12.796V3.204L4.519 8 10 12.796zm-.659.753-5.48-4.796a1 1 0 0 1 0-1.506l5.48-4.796A1 1 0 0 1 11 3.204v9.592a1 1 0 0 1-1.659.753z" />
                </svg></a>
            ...
            ?>
              ...
              <div class="pagenumborder">
                <div class="pagenum<?php echo $classname ?>">
                  <a href="?PageNum=<?php echo $i ?>&sort=<?php echo $sort ?>"><?php echo $i + 1 ?></a>
                </div>
              </div>
      ...
      $page = $_GET['PageNum'];
    } echo $page + 1 ?>&sort=<?php echo $sort ?>"> <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-caret-right" viewBox="0 0 16 16">
...
<?php } else { ?>
  ...
  <form method="">
    ...
    ?>
      <div style="display: none;">
        <a href="?PageNum=<?php echo $_REQUEST['PageNum'] - 1 ?>"> <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-caret-left" viewBox="0 0 16 16">
            <path d="M10 12.796V3.204L4.519 8 10 12.796zm-.659.753-5.48-4.796a1 1 0 0 1 0-1.506l5.48-4.796A1 1 0 0 1 11 3.204v9.592a1 1 0 0 1-1.659.753z" />
          </svg></a>
